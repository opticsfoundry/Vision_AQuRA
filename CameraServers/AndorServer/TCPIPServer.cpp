...
CTCPIPServer::CTCPIPServer(unsigned short* ImagePtrVal,unsigned short* FKSImagePtrVal) 
:CNetworkServer() {		
	LastTime=0;				
	HardwareAccess=true;	
	AndorServerDlg=NULL;
	...
	//for (int i=0;i<5;i++) Image[i]=NULL;
...
void CTCPIPServer::SetParameters() {	
	...
	long PrePictureL;
	long FlushWait;
	long DoInternalTriggerAfterExternalTrigger;
	...
	ExposureTime=((double)ExposureTimems)*0.001;  //conversion to seconds
	ok=ok && ReadInt(GoalTemperature);
	ok=ok && ReadInt(ExternalTrigger);	
	ok=ok && ReadInt(vSpeed);
	ok=ok && ReadInt(hSpeed);
	...
	ok=ok && ReadInt(DoInternalTriggerAfterExternalTrigger);
	ok=ok && ReadInt(NumberOfAdditionalReferenceImages);
	if (xMax>Andor->Width) {xMax=Andor->Width;error=1;}
	if (yMax>Andor->Height) {yMax=Andor->Height;error=2;}
	...
	Andor->SetParameters(hBin, vBin,xMin+1,xMax+1,yMin+1,yMax+1,
		ExposureTime,GoalTemperature, (ExternalTrigger==1),
		vSpeed,hSpeed, 
		(FKSMode==1),FKSHeight, FKSImages, FKSDataImages, FKSReferenceImages, FKSvSpeed, (FKSExternalTriggerStart==1),FKSDirtyLines,
		FlushWait, NumberPictures,PrePictureL==1,DoInternalTriggerAfterExternalTrigger==1,NumberOfAdditionalReferenceImages);
}
...
//	unsigned short* Image=Andor->ImagePtr;
//	const unsigned short Image[5][(1024*1024)];
	...
	//}
	//CString buf;
			//buf.Format("CAndor::TakePicture : num2");
			//AfxMessageBox(buf);
	if (ok) ok=Andor->TakePicture(); else error=1;
	//just for debug:
	//DWORD Start=GetTickCount();
	//while (abs(GetTickCount()-Start)<2000);
...
/*void CTCPIPServer::PrepareTransferImage() {
	long hBin;
	long vBin;
	long xMin;
	long xMax;
	...
	if (!FKSMode) {
		...
		//in FKS mode, the first picture contains the absorption and probe pictures
		...
		long SubImageSize=(Width*(FKSHeight-FKSDirtyLinesCut))/(vBin*hBin);		
		for (int i=0;i<NrImages;i++) {
			if (i<2) {
				if (ok) {
					if (FKSRotatedImage1) delete FKSRotatedImage1;
					...
					for (int x=0;x<MyHeight;x++) {
						for (int y=0;y<MyWidth;y++) {
							FKSRotatedImage1[x+y*MyHeight]=Image[i][pos];
							pos++;
						}
					}
				} else error=6;
			...
			if ((i==0) || (i==2)) {				
				if (ok) {					
					long Offset=(Width*FKSHeight)/(vBin*hBin);
					TakingImageStatus=3;
					UpdateStatus();	
					if (FKSRotatedImage2) delete FKSRotatedImage2;
					...
					for (int x=0;x<MyHeight;x++) {
						for (int y=0;y<MyWidth;y++) {
							FKSRotatedImage2[x+y*MyHeight]=Image[i][pos];
							pos++;
						}
					}
				} else error=8;
	...
	if (error!=0) {
		CString buf;
		buf.Format("CAndorServerDlg::PrepareTransferImage : error %i",error);
		AfxMessageBox(buf);
	}
}*/
...
//						long SubImageSize=FKSDataImages*(Width*(FKSHeight-FKSDirtyLinesCut))/(vBin*hBin);
	...
	if (!FKSMode){
		size=ceil(((double)((xMax-xMin+1)*(yMax-yMin+1)))/(double)(hBin*vBin));
	} else { 
		size=Width*Height;
	}
//	size=1002*1004;
...
//	else size=MyWidth*MyTotalHeight;

	Image=ImagePtr;
	for (j=0;j<(NrImages*ImageSize);j++) {
		*Image=*Image << 2;
		Image++;
	}
		...
		for (int i=0;i<(NrImages);i++) {
			...
			if (ok) {		
				...
				if (!FKSMode) {
					...
					SendData((unsigned char*)Image,size*2);
				}
				if (FKSMode){
				
					TakingImageStatus=3;
					UpdateStatus();	
					//WriteInteger(FKSImageSize);
					//FKSImage=FKSImagePtr+FKSImageSize*i;
					//SendData((unsigned char*)FKSImage,FKSImageSize*2);
					WriteInteger(1004*FKSHeight);
					Image=ImagePtr+ImageSize*i;
					SendData((unsigned char*)Image,1004*FKSHeight*2);
				}
...
//		int subimagenum=0;
//		long SubImageSize=FKSDataImages*(Width*(FKSHeight-FKSDirtyLinesCut))/(vBin*hBin);
//		int idataend=FKSDataImages;
//		if (PrePicture) idataend++;
//		for (int i=0;i<(NrImages);i++) {
//			subimagenum++;
//				Image[i]
//
...
//					int MyTotalHeight=FKSDataImages*MyHeight;
//					//We need to rotate for our camera orientation
//					long pos=Width*FKSHeight*subimagenum;
//					//long pos=0;
...
//	}
	...
	//for (i=0;i<NrImages;i++) if (Image[i]) {
	//	delete Image[i];
	//	Image[i]=NULL;
	//}
	TakingImageStatus=0;
