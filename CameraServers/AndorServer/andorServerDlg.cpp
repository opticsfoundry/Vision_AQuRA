...
//	unsigned short Image[5][(1024*1024)];
	...
	Andor->SetParameters(/*hBin*/ 1, /*vBin*/ 1, /*xMin*/ 1,/*xMax*/ 1004,/*yMin*/ 1,/*yMax*/ 1002,
		/*ExposureTime*/ 0.1,/*Temperature*/ 0,/*ExternalTrigger*/ false,
		/*vSpeed*/ 12.5, /*hSpeed*/ 1, /*FKSMode*/ true,/*FKSHeight*/ 334,/*FKSImages*/ 2,/*FKSDataImages*/ 1,/*FKSReferenceImages*/ 1,/*FKSvSpeed*/ 2,/*FKSExternalTriggerStart,*/ true,
		/*FKSDirtyLines*/0,/*FlushWait*/ 100, /*NumberPictures*/ NrImages,/*PrePicture*/false,/*DoInternalTriggerAfterExternalTrigger*/ false,/*NumberOfAdditionalReferenceImages*/0);
//	unsigned short* Image[NrImages];	
//	for (i=0;i<NrImages;i++) Image[i]=new unsigned short[(1024*1024)];
	//CString buf;
			//buf.Format("CAndor::TakePicture : ready now");
			//AfxMessageBox(buf);
	Andor->TakePicture();	
...
void CAndorServerDlg::OnExposeExternal() 
{
	WakeUp();
	const int NrImages=1;
//	unsigned short* Imageptr=CTCPIPServer::Image[0][0];
	int i=0;
	Andor->SetParameters(/*hBin*/ 1, /*vBin*/ 1, /*xMin*/ 1,/*xMax*/ 512,/*yMin*/ 1,/*yMax*/ 512,
		/*ExposureTime*/ 0.1,/*Temperature*/ 0,/*ExternalTrigger*/ true,
		/*vSpeed*/ 16, /*hSpeed*/ 1, /*FKSMode*/ false,/*FKSHeight*/ 512,/*FKSImages*/ 2,/*FKSDataImages*/ 1,/*FKSReferenceImages*/ 1,/*FKSvSpeed*/ 2,/*FKSExternalTriggerStart,*/ true,
		/*FKSDirtyLines*/0,/*FlushWait*/ 100, /*NumberPictures*/ NrImages,/*PrePicture*/false,/*DoInternalTriggerAfterExternalTrigger*/ false,/*NumberOfAdditionalReferenceImages*/0);
	//unsigned short* Image[NrImages];	
	//for (i=0;i<NrImages;i++) Image[i]=new unsigned short[(1024*1024)/(1*1)];
	//CString buf;
	//		buf.Format("CAndor::TakePicture : num1");
	//		AfxMessageBox(buf);
	Andor->TakePicture();
//	for (i=0;i<NrImages;i++) {
//		delete[] Image[i];
